...
function assessTotal(game, sum) {
  ...
  const gap = proj.total - res.cons;
  const over = gap > 0;
  const best = over ? res.bestA : res.bestB;
  const line = best.L;
  const pOver = 1 - normCdf((line - proj.total) / SIG_T);
  ...
  if (Math.abs(gap) < 3.5)
    fail.push(`projection is ${Math.abs(gap).toFixed(1)} points off the total, under the 3.5 required`);
  if (ev < 0.02)
    fail.push(`only ${(ev * 100).toFixed(1)}% expected value even if the projection is right`);
...
function assessML(game, sum) {
  ...
  if (edge < 0.04) fail.push(`only ${(edge * 100).toFixed(1)} points of win probability between model and market`);
...
function CardTab({ board, boardOdds, entries, onOpen }) {
  ...
  const reasons = (c) => (
    <ul className="why">
      {c.kind === "total" ? (
        <>
          <li>Scoring rates project {c.proj.toFixed(1)}, {Math.abs(c.gap).toFixed(1)} points
              {c.over ? " above" : " below"} the market's {trim(c.res.cons)} across {c.res.count}
    ...
    <>
      ...
      {swept && (
        <div className="stale" style={{ borderLeftColor: plays.length ? "#35D07F" : "#7E8899" }}>
          ...
          {oneBook > cands.length * 0.6 && (
            <> <b style={{ color: "#E3B448" }}>Most of these have a single book.</b> ESPN's feed
            carries one price on small games, and one price isn't a market — that's what's thinning
            this card, not the thresholds.</>
          )}{" "}
          {plays.length
      ...
      {plays.length > 0 && <p className="tier">Plays · cleared every test</p>}
      {plays.map((c, i) => (
      ...
      {swept && (
        <p className="empty">
          Spread and moneyline numbers assume ESPN's projection is right; totals assume the scoring
          model is. Both are single models with no second opinion behind them. A play clearing the

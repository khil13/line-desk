...
function CardTab({ board, boardOdds, entries, onOpen }) {
      ...
      {c.kind === "total" ? (
        <>
          <li>Scoring rates project {c.proj.toFixed(1)}, {Math.abs(c.gap).toFixed(1)} points
              {c.over ? " above" : " below"} the market's {trim(c.res.cons)} across {c.res.count} books.</li>
          <li>Lands this side {(c.pModel * 100).toFixed(0)}% of the time if that holds —
              <b> {c.ev >= 0 ? "+" : ""}{(c.ev * 100).toFixed(1)}%</b> at {fmtOdds(c.price)}.</li>
          <li>Built on {c.games} games of scoring for the thinner of the two teams.</li>
        </>
      ) : c.kind === "spread" ? (
        <>
        ...
        <>
          <li>Model has this side winning {(c.pModel * 100).toFixed(0)}% outright, {(c.edge * 100).toFixed(1)}
              points above the market across {c.res.count} books.</li>
  ...
  return (
    <>
      ...
      {plays.map((c, i) => (
        <div className="playcard" key={c.kind + c.game.id} onClick={() => onOpen(c.game.id)}>
      ...
      {leans.length > 0 && (
        <>
          ...
          {leans.map((c) => (
            <div className="playcard lean" key={c.kind + c.game.id} onClick={() => onOpen(c.game.id)}>
              <div className="pbody">
                {headline(c)}
                <span className="pbook">{c.best.bk.n} · {c.game.away} at {c.game.home}</span>
      ...
      {watch.length > 0 && (
        <>
          ...
          {watch.map((c) => (
            <div className="gm" key={c.kind + c.game.id} style={{ borderLeftColor: tc(c.game, "h") }}
      ...
      {swept && !(EMP && EMP.teams) && (
        <p className="empty">
          <b style={{ color: "#E3B448" }}>Totals aren't being judged.</b> Over/under needs a scoring
          model and there isn't one until you calibrate — the settings tab builds it from real results
          at the same time it measures the key numbers.
        </p>
      )}
